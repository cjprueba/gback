# PowerShell Deployment Script for Gestor Backend (Docker Image)

param(
    [Parameter(Position=0)]
    [ValidateSet("deploy", "status", "delete", "build-only")]
...
function Deploy-Image {
    ...
    # Deploy application with custom image
    kubectl apply -f deployment-image.yaml
...
function Remove-Deployment {
    ...
    kubectl delete -f deployment-image.yaml --ignore-not-found=true
...
switch ($Action) {
    ...
    default {
        Write-Host "Usage: .\deploy-image.ps1 {deploy|status|delete|build-only}" -ForegroundColor Cyan
        Write-Host "  deploy     - Build image and deploy to default namespace" -ForegroundColor White
        Write-Host "  status     - Check deployment status" -ForegroundColor White
        Write-Host "  delete     - Delete deployment" -ForegroundColor White
        Write-Host "  build-only - Build Docker image only" -ForegroundColor White
        exit 1
    }
}
