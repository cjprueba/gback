# PowerShell Infrastructure Deployment Script for Gestor Backend
...
function Remove-Deployment {
    Write-Warning "Deleting infrastructure deployment..."
    kubectl delete -f . --ignore-not-found=true
    kubectl delete namespace gestor-backend --ignore-not-found=true
    Write-Status "Infrastructure deployment deleted"
}
...
switch ($Action) {
    ...
    default {
        Write-Host "Usage: .\deploy-infrastructure.ps1 {deploy|status|delete}" -ForegroundColor Cyan
        Write-Host "  deploy  - Deploy infrastructure (PostgreSQL + MinIO)" -ForegroundColor White
